...
def add_table_full():
    ...
    try:
        if e4_var.get()=="":
            ...
            status=l.add_tables(e1_var.get(),e2_var.get(),e3_var.get(),e4_var.get())
    except:
        messagebox.showinfo("Error","Number fields should be with number only!!")
        
    if status=="e2":
        messagebox.showinfo("Error","Table no is already registerd!!")
        e1.delete(0,END)
...
b2=Button(f1,text="Add Table",bg="grey",fg="black",font=("times",20,"bold"),command=add_table_full)
